...
+(void)updateWithAPPID:(NSString *)appId withBundleId:(NSString *)bundelId{
    ...
    [AppUpdate updateWithAPPID:appId withBundleId:bundelId block:^(NSString *currentVersion, NSString *storeVersion, NSString *openUrl, BOOL isUpdate) {
        
        if (isUpdate) {
            [AppUpdate showAlertViewTitle:@"新版本提醒" subTitle:[NSString stringWithFormat:@"检测到新版本%@,是否更新？",storeVersion] openUrl:openUrl];
        }else{
